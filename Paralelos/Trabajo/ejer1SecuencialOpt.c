...
for (i = 0; i < N; i++){
  for (j = 0; j <= i; j++){
  	zonaTriangular = j+(i*(i+1)/2); //indice del vector que se multiplicará con valor de B
  	//printf("zona triangular: %d\n", zonaTriangular);
    for (k = 0; k < N; k++){
    	LB[i*N+k] = LB[i*N+k] + LT[zonaTriangular]*B[k+j*N];
      //printf("Zona indice de triangular: %d con zona matriz B: %d\n y guardando en matriz LB zona: %d\n", zonaTriangular, k+j*N, i*N+k);
    }
  }
}
