...
#pragma omp parallel for shared(A,B,AB) private(i,j,k)
for(i=0;i<N;i++){
  for(j=0;j<N;j++){
   for(k=0;k<N;k++){
...
#pragma omp parallel for shared(LB,LT,B) private(i,j,k, indiceTriangular)
for (i = 0; i < N; i++){
  for (j = 0; j <= i; j++){
...
#pragma omp parallel for shared(AB,C,ABC,LB,D,LBD) private(i,j,k)
for(i=0;i<N;i++){
  for(j=0;j<N;j++){
   for(k=0;k<N;k++){
