...
int resultados[16]={0,0,0,2,10,4,40,92,352,724,2680,14200,73712,365596,2279184,14772512};
...
int master(int slaves,int size, MPI_Status estado){
	...
	for (x=0; x<slaves; ++x) {
		MPI_Recv(&resp, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &estado);
	...
	while(i<size){ //estas van a ser las tareas, el indice i es lo que le voy a pasar al hilo worker 
		...
		if(flag && i<size){ //flag = true si tengo pedidos
			MPI_Recv(&resp, 1, MPI_INT, estado.MPI_SOURCE, estado.MPI_TAG, MPI_COMM_WORLD, &estado);
			soluciones +=resp;
			MPI_Send(&i, 1, MPI_INT, estado.MPI_SOURCE, 0, MPI_COMM_WORLD);
			i++;
		}else{	//Trabaja master
	...
	int terminar= -999;
	printf("No hay mas pedidos. i:%i sinTrabajo: %i\n",i,sinTrabajo);
	for (x=sinTrabajo; x<slaves; x++) {
		//printf("hago recive\n");
		MPI_Recv(&resp, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &estado);
		//printf("x: %i\n", x);
		if(resp>0){ //Si es -1 es porque nunca pudo trabajar, porque son muchos hilos o porque les ganó el master
			soluciones +=resp;
		}
		//printf("hago el send\n");
...
void slave(int id, MPI_Status estado, int size){
	...
	if(tarea<0){
		printf("Slave %i: no trabaje\n",id);
	}
	while(tarea>=0){
		//Trabajar
		printf("Slave %i trabajando\n",id);
	...
	printf("Slave %i. Tiempo: %g segundos\n",id,dwalltime() - start_time);
}
int main(int argc, char** argv) {
	...
	double start_time = dwalltime();
	if (identificador == 0) {
		soluciones=master(hilos-1,size,estado);
		printf("Tiempo: %g segundos\n", dwalltime() - start_time);
		//Checkear resultado
		if (resultados[size-1]==soluciones) {
			printf("Soluciones: %d\nOK\n", soluciones);
		} else {
			printf("Soluciones: %d\nERROR\n", soluciones);
		}
